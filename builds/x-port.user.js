// ==UserScript==
// @name        X-Porter
// @namespace   X-Porter
// @namespace   dnsev-h (original)
// @description Export favorites on E*Hentai
// @match       https://e-hentai.org/favorites.php*
// @match       https://exhentai.org/favorites.php*
// @updateURL   https://raw.githubusercontent.com/darkred3/x-porter/refs/heads/master/builds/x-port.meta.js
// @downloadURL https://raw.githubusercontent.com/darkred3/x-porter/refs/heads/master/builds/x-port.user.js
// @icon        data:image/png;base64,
// @icon64      data:image/png;base64,
// @grant       none
// @run-at      document-idle
// @version     1.1.1
// ==/UserScript==

(function () {
  'use strict';

  const sleep = (ms) => new Promise((res) => setTimeout(res, ms));
  let includeTitles = true;
  let entries = [];

  function parseFavoritesDocument(doc) {
    const results = [];
    const items = doc.querySelectorAll("div.gl1t");

    for (const item of items) {
      const linkEl = item.querySelector(".gl4t a[href]");
      const titleEl = item.querySelector(".glink");

      if (linkEl && titleEl) {
        results.push({
          url: linkEl.href,
          title: titleEl.textContent.trim(),
        });
      }
    }
    return results;
  }

  async function fetchAllPages(firstUrl, progressEl) {
    entries = [];
    let nextUrl = firstUrl;

    while (nextUrl) {
      const res = await fetch(nextUrl);
      const text = await res.text();
      const doc = new DOMParser().parseFromString(text, "text/html");

      const newEntries = parseFavoritesDocument(doc);
      if (newEntries.length === 0) break;

      entries.push(...newEntries);

      progressEl.textContent = `Total: ${entries.length}`;

      const nextLink = doc.querySelector("a#unext");
      nextUrl = nextLink?.href || null;
      if (nextUrl) await sleep(1000); // Be polite
    }
  }

  function renderExportUI() {
    const container = document.createElement("div");
    container.style.marginTop = "10px";
    container.style.padding = "10px";
    container.style.border = "1px solid #ccc";
    container.style.borderRadius = "4px";

    const controls = document.createElement("div");
    controls.style.marginBottom = "6px";

    const makeLink = (text) => {
      const link = document.createElement("a");
      link.href = "#";
      link.textContent = text;
      link.style.marginRight = "12px";
      return link;
    };

    const showListBtn = makeLink("Show List");
    const copyBtn = makeLink("Copy");
    const downloadBtn = makeLink("Download");
    const toggleTitlesBtn = makeLink("Include Titles");

    const totalDisplay = document.createElement("span");
    totalDisplay.textContent = "Total: 0";
    totalDisplay.style.marginLeft = "10px";

    const listWindow = document.createElement("textarea");
    listWindow.style.display = "none";
    listWindow.style.width = "100%";
    listWindow.rows = 10;
    listWindow.style.marginTop = "10px";
    listWindow.style.padding = "6px 10px";
    listWindow.style.boxSizing = "border-box";


    function updateListWindow() {
      const lines = entries.map(e =>
        includeTitles ? `${e.url} - ${e.title}` : e.url
      );
      listWindow.value = lines.join("\n");
    }

    showListBtn.onclick = (e) => {
      e.preventDefault();
      updateListWindow();
      listWindow.style.display = listWindow.style.display === "none" ? "block" : "none";
    };

    copyBtn.onclick = (e) => {
      e.preventDefault();
      updateListWindow();
      listWindow.select();
      document.execCommand("copy");
    };

    downloadBtn.onclick = (e) => {
      e.preventDefault();
      updateListWindow();
      const blob = new Blob([listWindow.value], { type: "text/plain" });
      const a = document.createElement("a");
      a.href = URL.createObjectURL(blob);
      a.download = "favorites.txt";
      a.click();
    };

    toggleTitlesBtn.onclick = (e) => {
      e.preventDefault();
      includeTitles = !includeTitles;
      toggleTitlesBtn.textContent = includeTitles ? "Include Titles" : "Omit Titles";
      updateListWindow();
    };

    controls.append(showListBtn, copyBtn, downloadBtn, toggleTitlesBtn, totalDisplay);
    container.appendChild(controls);
    container.appendChild(listWindow);

    return { container, totalDisplay };
  }




  function addExportButton() {
    const buttonRef = document.querySelector('input[type="button"][value="Clear"]');
    if (!buttonRef) return;

    const exportBtn = document.createElement("input");
    exportBtn.type = "button";
    exportBtn.value = "Export...";
    exportBtn.style.marginLeft = "8px";
    exportBtn.style.marginTop = "4px";

    let exportUI = null; // keep reference to the UI container

    exportBtn.addEventListener("click", async () => {
      if (exportUI) {
        // UI already exists — remove it and reset
        exportUI.remove();
        exportUI = null;
        exportBtn.disabled = false;
        exportBtn.value = "Export...";
        return;
      }

      exportBtn.disabled = true;
      exportBtn.value = "Exporting...";

      const { container, totalDisplay } = renderExportUI();
      exportUI = container;
      buttonRef.parentNode.appendChild(exportUI);

      await fetchAllPages(window.location.href, totalDisplay);

      exportBtn.disabled = false;
      exportBtn.value = "Export...";
    });

    buttonRef.parentNode.insertBefore(exportBtn, buttonRef.nextSibling);
  }


  addExportButton();
})();
